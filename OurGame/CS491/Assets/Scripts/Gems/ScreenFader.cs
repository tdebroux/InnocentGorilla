...
public class ScreenFader : MonoBehaviour
{
    ...
    private void Start()
    {
        anim = GameObject.FindGameObjectWithTag("ScreenFaderTag").GetComponent<Animator>();
        StartFade();
    }
    void Awake()
    {
        //FadeImg.rectTransform.localScale = new Vector2(1024, 768);
        //FadeImg.color = Color.clear;
    }

    public void StartFade()
    {
        anim.SetInteger("FaderInt", 1);
        StartCoroutine("ResetFaderInt", 1.75);

    }

    public IEnumerator ResetFaderInt(float seconds)
    {
        yield return new WaitForSeconds(seconds);
        anim.SetInteger("FaderInt", 0);
    }
    //void Update()
    //{
